...
def get_currency_symbols(api_key=None):
    ...
    if api_key:
        base_url = f"https://v6.exchangerate-api.com/v6/cbee27828ca6903f8d148ae4/latest/USD"
        try:
...
def get_exchange_rate(from_currency, to_currency, api_key=None):
    ...
    if api_key:
        base_url = f"https://v6.exchangerate-api.com/v6/cbee27828ca6903f8d148ae4/latest/{from_currency}"
        try:
            response = requests.get(base_url)
            response.raise_for_status()
            data = response.json()
...
# --- Historical Trend Chart using NBP Data ---
...
if st.button("Show Trend Chart 📊"):
    ...
    if historical_rates_data and len(historical_rates_data) > 0:
        ...
        fig = px.line(df_chart, x="Date", y="Rate", title=f"{from_currency} to {to_currency} Exchange Rate Trend (NBP)")
...
st.markdown("### Check Out the Latest Exchange Rates with respect to Pakistani Currency🌐")
st.markdown(
...
st.markdown(
    """
    <div style="position: fixed; bottom: 10px; right: 10px; font-size: 14px; color: #666666;">
    Made with ❤️ by <a href="XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX" target="_blank" style="color: #666666;">Syed Zeeshan Iqbal</a>
    </div>
    """,
    unsafe_allow_html=True,
)